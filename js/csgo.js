// Trigger awal menggunakan SweetAlert tanpa musik
window.addEventListener('load', () => {
    Swal.fire({
        title: 'Happy Birthday!', // Teks judul diganti
...
const animationTimeline = () => {
    ...
    // Simpan konten asli untuk Replay
    if (!textBoxChars.getAttribute('data-original')) {
        textBoxChars.setAttribute('data-original', textBoxChars.innerHTML);
    }
    if (!hbd.getAttribute('data-original')) {
        hbd.setAttribute('data-original', hbd.innerHTML);
    }

    const resetText = () => {
        textBoxChars.innerHTML = textBoxChars.getAttribute('data-original');
        hbd.innerHTML = hbd.getAttribute('data-original');
    };
    ...
    tl.to(".container", 0.6, { visibility: "visible" })
      ...
      .to(".button-wrapper", 1, { // Munculkan tombol di akhir
          opacity: 1,
          visibility: "visible",
          ease: Expo.easeOut
      }, "+=0.5");
    ...
    // Fitur Replay
    const replyBtn = document.getElementById("btnReplay");
